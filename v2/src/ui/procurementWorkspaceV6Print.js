...
export function renderPrintModal({ list, businessName }) {
  const typeLabel = list.kind === 'ORDER'
    ? 'LISTA DE PEDIDOS'
    : 'LISTA DE COMPRAS';
  ...
  return `
    <div class="v6p-modal v6p-print-modal">
      <div class="v6p-modal-head">
        <div>
          <div class="v6p-eyebrow">PREVISUALIZACIÓN 80MM</div>
          <h3>Así saldrá en la RC-8002</h3>
          <p>
            La vista conserva el formato visual; el botón envía ESC/POS
            directo por TCP/IP y corta cada copia.
          </p>
        </div>
        <button
          class="v6p-ghost"
          data-v6p-action="close-print"
          type="button"
        >Cerrar</button>
      </div>
      ...
      <div class="v6p-modal-body">
        <div class="v6p-print-toolbar">
          <div class="v6p-printer-hint">
            <strong>RC-8002 · 80mm · ESC/POS directo</strong><br>
            La calibración e IP viven en Configuración → Impresión 80mm.
          </div>
          <button
            class="v6p-primary"
...
function ticket({
  ...
  return `
    <section class="v6p-ticket ${
      ...
      <div class="v6p-ticket-notes">
        <b>Observaciones:</b>
        <i></i><i></i><i></i>
      </div>
      <div class="v6p-ticket-rule is-solid"></div>
      <footer>
        ${esc(typeLabel)} · ${copy}<br>
...
function ticketRow(item) {
  ...
  return `
    <div class="v6p-ticket-row">
      ...
      <b>${esc(
        formatNumber(display.quantity)
      )} ${esc(shortUnit(display.unit))}</b>
      <i></i>
    </div>
  `;
}
