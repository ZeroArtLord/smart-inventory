...
export function buildSaintInitialLoadInstructionsMatrix() {
  return [
    ...
    [
      'SKU SMART',
      'Código interno de control en Smart Inventory. Es opcional: si lo dejas vacío se genera automáticamente desde CÓDIGO SAINT, por ejemplo SM-REF001. No es la clave del futuro Bridge SAINT.',
      'SM-REF001'
    ],
    [
      'PRODUCTO',
...
export function parseCatalogMatrix(matrix) {
  ...
  for (let index = startRow; index < matrix.length; index++) {
    ...
    const saintCode =
      normalizeText(
        raw[columns.saintCode]
      );
    const explicitSku =
      normalizeText(
        raw[columns.sku]
      );
    const hasExplicitSku =
    ...
    if (
      columns.saintCode !== undefined &&
      !saintCode
    ) {
      errors.push(
        `Fila ${excelRow}: falta CÓDIGO SAINT`
      );
      continue;
    }
    ...
    if (
      columns.saintCode !== undefined &&
      typeof raw[columns.saintCode] === 'number'
    ) {
      warnings.push(
        `Fila ${excelRow}: CÓDIGO SAINT llegó como número; verifica que SAINT/Excel no haya eliminado ceros a la izquierda.`
      );
    }
  ...
  for (const row of preview.rows) {
    try {
      ...
      if (existing) {
        ...
        if (hasColumn('saintCode')) {
          patch.saintCode =
            row.saintCode;
        }
        if (row.hasExplicitSku) {
          patch.sku = row.sku;
        } else if (
          !existing.sku &&
          row.skuAutoGenerated
        ) {
          patch.sku = row.sku;
        }
