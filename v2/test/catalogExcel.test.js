...
test('interpreta empaques SAINT y convierte mínimos/máximos a unidad base', () => {
  ...
  assert.equal(row.saintCode, 'REF001');
  assert.equal(row.sku, 'SM-REF001');
  assert.equal(row.skuAutoGenerated, true);
...
test('la plantilla SAINT incluye instrucciones de seguridad y unidades soportadas', () => {
  ...
  assert.match(
    text,
    /SKU SMART.*genera automáticamente/i
  );
});
...
test('Código SAINT y SKU Smart son identidades distintas', () => {
  const preview = parseCatalogMatrix([
    [
      'Producto',
      'Código SAINT',
  ...
  assert.equal(
    preview.rows[0].skuAutoGenerated,
    false
  );
});

test('si SKU Smart está vacío se genera de forma determinista desde Código SAINT', () => {
  const preview = parseCatalogMatrix([
    [
      'Producto',
      'Código SAINT',
...
test('si existe columna Código SAINT una fila usada no puede dejarla vacía', () => {
  const preview = parseCatalogMatrix([
    [
      'Producto',
      'Código SAINT',
